    ...
    override suspend fun fetchContacts(): List<RemoteContact> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (statusCode !in 200..299) {
                throw ContactsApiException("Contacts BFF responded with status $statusCode.")
            }
    ...
    override suspend fun fetchContactById(id: String): RemoteContact? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            when {
                ...
                statusCode !in 200..299 -> throw ContactsApiException("Contacts BFF responded with status $statusCode.")
                else -> {
                    val body = connection.inputStream.bufferedReader().use { it.readText() }
