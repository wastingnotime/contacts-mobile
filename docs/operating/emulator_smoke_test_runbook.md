...
Keep the BFF code owned by this repo while making the emulator launch path explicit and separate from the Android client repo.

The emulator smoke-test backend and local launch orchestration should come from `../runtime-sandbox`.
That sandbox owns the local execution path used for emulator validation, including starting the sandbox-backed stack that the Android app talks to.
...
Use the runtime sandbox validation path before launching the Android app:

1. Use the sandbox command that starts the backend and the mobile BFF for validation.
2. Install the Android app with the emulator base URL that the sandbox-backed stack exposes.
3. Open the app on the emulator and verify the contacts list loads.

## Android Client Smoke Test

1. Confirm the sandbox-backed validation stack is ready in `../runtime-sandbox`.
...
| --- | --- |
| Sandbox-backed validation stack in `../runtime-sandbox` | The backend and BFF launch together, health becomes reachable, and seeded contacts are available. |
...
| The app launches but cannot reach the contacts API | The emulator base URL is misconfigured or the sandbox-backed stack is unavailable. |
...
- use `../runtime-sandbox` for deterministic validation setup and smoke checks
- the BFF runtime runbook is for standalone BFF checks, not the emulator validation path
- the sandbox owns the local execution path for emulator validation, while this repo owns the BFF code and contract
