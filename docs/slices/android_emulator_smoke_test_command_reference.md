...
Turn the emulator smoke-test runbook into a concrete command reference that points at the adjacent runtime sandbox repo for backend simulation and uses explicit command names rather than generic guidance.
...
- the Android client should be validated only after the backend smoke path is live
...
- the command reference should stay short and repeatable
...
- verify the command reference keeps the Android client validation sequence clear
