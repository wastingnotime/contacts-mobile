...
- load the selected contact from `GET /api/contacts/{id}` through the repository-owned Go BFF
...
It loads `GET /api/contacts/{id}`, maps the transport payload into app models, and returns a state the UI can render through the repository-owned Go BFF.

## Main Business Rules

- contact detail is read from the backend through the repository-owned BFF, not reconstructed only from the list item
...
- verify tapping a contact in the list navigates to the repository-owned BFF-backed detail loading flow
...
- the detail screen can render a contact from the repository-owned BFF contract
