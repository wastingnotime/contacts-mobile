...
- keep `MainActivity` dependent on a generic app-start object instead of repository-owned BFF-specific naming
...
The use cases should still run exactly as before, but the activity should call an app-level startup facade instead of a repository-owned BFF-specific startup name.

## Main Business Rules

- startup naming should reflect the app boundary, not the transport boundary
- the activity should not know repository-owned BFF-specific startup types
...
- verify `MainActivity` can depend on the app-start facade instead of repository-owned BFF-specific naming
